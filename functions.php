...
function blankslate_enqueue() {
    ...
    wp_enqueue_style('theme_styles', get_template_directory_uri().'/assets/css/theme.css');
    wp_enqueue_style('swiper_styles', get_template_directory_uri().'/assets/plugins/swiper/swiper-bundle.min.css');
    wp_enqueue_script( 'jquery' );
    wp_enqueue_script( 'swiper_scripts', get_template_directory_uri().'/assets/plugins/swiper/swiper-bundle.min.js');
    wp_enqueue_script( 'theme_scripts', get_template_directory_uri().'/assets/js/theme.js');

    $schema = is_ssl() ? 'https':'http';
    wp_localize_script('theme_scripts', 'myAjax', array(
...
// Add wp_two_factor table
function create_two_factor_table()
{
    global $wpdb;
    $two_factor_table = $wpdb->get_blog_prefix() . 'two_factor_table';

    if ($wpdb->get_var("SHOW TABLES LIKE '$two_factor_table'") != $two_factor_table) {
        require_once ABSPATH . 'wp-admin/includes/upgrade.php';
        $charset_collate = "DEFAULT CHARACTER SET {$wpdb->charset} COLLATE {$wpdb->collate}";
        $sql = "CREATE TABLE {$two_factor_table} (
        user_mail varchar (255),
        two_factor_code int (4)
        ) {$charset_collate};";
        dbDelta($sql);
    }
}
